...
mysvy <- function(data, columns, ...) {
    model <- lapply(as.list(columns), function(x) {
          svyglm(as.formula(paste0(names(data)[x], "~class + rcs(age, 3) + rcs(year, 3)")), 
            data = data, family=poisson(), ...)
        })
    return(model)
}

results <- mysvy(dat, 124:133, design = svy_dat)

regs = NULL
for(i in 1:10){
  cbind(exp(results[[i]]$coefficients[2:4]), exp(confint(results[[i]])[2:4,])) -> bind
  rbind(regs, cbind(bind, rep(names(dat)[124:133][i],3))) -> regs
}

regs %>%
  as.data.frame() %>%
  mutate(Class=factor(rep(c("Managers", "Petit bourgeoisie", "Capitalists"), 10), levels=c("Managers", "Petit bourgeoisie", "Capitalists")),
         `Prevalence ratio`=as.numeric(V1),
         Lower=as.numeric(`2.5 %`),
         Upper=as.numeric(`97.5 %`)) -> regs

ggplot(regs, aes(y=Class, x=`Prevalence ratio`, xmin=Lower, xmax=Upper)) +
  geom_point() +
  geom_errorbar() + 
  ...
  scale_y_discrete(limits=rev(levels(regs$Class))) +
  theme_light() + 
...
mysvy <- function(data, columns, ...) {
    model <- lapply(as.list(columns), function(x) {
          svyglm(as.formula(paste0(names(data)[x], "~race + rcs(age, 3) + rcs(year, 3)")), 
            data = data, family=poisson(), ...)
        })
    return(model)
}

results <- mysvy(dat, 124:133, design = subset(svy_dat, class=="Workers"))

regs = NULL
for(i in 1:10){
  cbind(exp(results[[i]]$coefficients[2:3]), exp(confint(results[[i]])[2:3,])) -> bind
  rbind(regs, cbind(bind, rep(names(dat)[124:133][i],2))) -> regs
}

regs %>%
  as.data.frame() %>%
  mutate(Race=factor(rep(c("Black", "Other"), 10), levels=c("Black", "Other")),
         `Prevalence ratio`=as.numeric(V1),
         Lower=as.numeric(`2.5 %`),
         Upper=as.numeric(`97.5 %`)) -> regs

ggplot(regs, aes(y=Race, x=`Prevalence ratio`, xmin=Lower, xmax=Upper)) +
  geom_point() +
  geom_errorbar() + 
  geom_vline(xintercept=1, lty=2, col='darkgrey') +
  facet_grid(V4~.) +
  scale_x_continuous(trans="log", limits=c(0.3, 2.5), breaks=c(0.3, 0.65, 1, 1.35, 1.7, 2.05, 2.4)) +
  scale_y_discrete(limits=rev(levels(regs$Class))) +
  theme_light() + 
  theme(strip.background = element_rect(fill=NA, color='grey'), strip.text = element_text(colour = "black"))
```