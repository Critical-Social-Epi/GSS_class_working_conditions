...
* Exclude IAP in 2006 and 2014 (even those with wrkstat 1-3 and correct ballots), per advice of GSS help desk, as they weren't asked the QWL variables for various reasons. Consider importing data using alternative program or downloading data in alternative format to preserve IAP labels.
...
aggr(dat[,c("class", 'sex', 'race', 'educ', 'marital_tri', 'region', 'income', 'age', "srh_bin", "mntlhlth", "any_disc_haras", "respect_bin", "trustman_bin", "manvsemp_bin", "suphelp_bin", "supcares_bin", "wkfreedm_bin", "wkdecide_bin", "safehlth_bin", "safefrst_bin")], cex.axis = 0.7, numbers=FALSE, prop=TRUE)
```
...
```{r warning=FALSE, message=FALSE}
#vars of interest
qwl_vars <- c("any_disc_haras", "respect_bin", "trustman_bin", "manvsemp_bin", "suphelp_bin", "supcares_bin", "wkfreedm_bin", "wkdecide_bin", "safehlth_bin", "safefrst_bin")
...
#fully adjusted
full_adj <- mysvy(dat, 124:133, "~class + rcs(age, 3) + rcs(year, 3) + sex + race + region + educ + marital_tri", design = svy_dat)
...
#bind together
binded <- rbind(cbind(regs_less, rep("Age+year", 30)), cbind(regs_more, rep("Age+year+gender+race", 30)), cbind(regs_full, rep("Age+year+gender+race+educ+region+marital", 30)))

#format
binded %>%
  as.data.frame() %>%
  mutate(Class=factor(rep(c("Managers", "Petit bourgeoisie", "Capitalists"), 30), levels=c("Managers", "Petit bourgeoisie", "Capitalists")),
         V4=factor(V4, levels=c("any_disc_haras", "respect_bin", "trustman_bin", "manvsemp_bin", "suphelp_bin", "supcares_bin", "wkfreedm_bin", "wkdecide_bin", "safehlth_bin", "safefrst_bin")),
         V5=factor(V5, levels=c("Age+year", "Age+year+gender+race", "Age+year+gender+race+educ+region+marital")),
...
#fully adjusted men
full_adj <- mysvy(dat, 124:133, "~class*sex + rcs(age, 3) + rcs(year, 3) + race + region + educ + marital_tri", design = svy_dat)
...
full_adj <- mysvy(dat, 124:133, "~class*relevel(sex, 'female') + rcs(age, 3) + rcs(year, 3) + race + region + educ + marital_tri", design = svy_dat)
...
binded <- rbind(cbind(regs_less_men, rep("Age+year", 30)), cbind(regs_less_women, rep("Age+year", 30)), 
                ...
                cbind(regs_full_men, rep("Age+year+race+educ+region+marital", 30)), cbind(regs_full_women, rep("Age+year+race+educ+region+marital", 30)))
...
binded %>%
  ...
  mutate(Class=factor(rep(c("Managers", "Petit bourgeoisie", "Capitalists"), 60), levels=c("Managers", "Petit bourgeoisie", "Capitalists")),
         V4=factor(V4, levels=c("any_disc_haras", "respect_bin", "trustman_bin", "manvsemp_bin", "suphelp_bin", "supcares_bin", "wkfreedm_bin", "wkdecide_bin", "safehlth_bin", "safefrst_bin")),
         V5=factor(V5, levels=c("Age+year", "Age+year+race", "Age+year+race+educ+region+marital")),
...
ggplot(binded, aes(y=Class, x=`Prevalence ratio`, xmin=Lower, xmax=Upper, group=rev(V5), shape=V5, color=V5)) +
  ...
  scale_x_continuous(trans="log", limits=c(0.015, 2.31), breaks=c(0.1, 0.55, 1, 1.45, 1.9, 2.35), labels=c("0.1", "0.55", "1.0", "1.45", "1.9", "2.35")) +
  scale_y_discrete(limits=rev(levels(binded$Class))) +
  theme_light() + 
  scale_color_manual(values=brewer.pal(8, "Blues")[c(4,6,8)]) +
  theme(strip.background = element_rect(fill=NA, color='grey'), strip.text = element_text(colour = "black"), legend.position='bottom', legend.title=element_blank())
...
#full adj
full_adj <- mysvy(dat, 124:133, "~race + rcs(age, 3) + rcs(year, 3) + sex + educ + region + marital_tri", design = subset(svy_dat, class=="Workers"))
...
#bind together
binded <- rbind(cbind(regs_less, rep("Age+year", 20)), cbind(regs_more, rep("Age+year+gender", 20)), cbind(regs_full, rep("Age+year+gender+educ+region+marital", 20)))
...
binded %>%
  ...
  mutate(Race=factor(rep(c("Black", "Other"), 30), levels=c("Black", "Other")),
         V4=factor(V4, levels=c("any_disc_haras", "respect_bin", "trustman_bin", "manvsemp_bin", "suphelp_bin", "supcares_bin", "wkfreedm_bin", "wkdecide_bin", "safehlth_bin", "safefrst_bin")),
         V5=factor(V5, levels=c("Age+year", "Age+year+gender", "Age+year+gender+educ+region+marital")),
         `Prevalence ratio`=as.numeric(V1),
         Lower=as.numeric(`2.5 %`),
         Upper=as.numeric(`97.5 %`)) -> binded
...
#fully adjusted men
full_adj <- mysvy(dat, 124:133, "~race*sex + rcs(age, 3) + rcs(year, 3) + region + educ + marital_tri", design = subset(svy_dat, class=="Workers"))
...
#fully adjusted women
full_adj <- mysvy(dat, 124:133, "~race*relevel(sex, 'female') + rcs(age, 3) + rcs(year, 3) + region + educ + marital_tri", design = subset(svy_dat, class=="Workers"))
...
binded <- rbind(cbind(regs_less_men, rep("Age+year", 20)), cbind(regs_less_women, rep("Age+year", 20)), 
                cbind(regs_full_men, rep("Age+year+educ+region+marital", 20)), cbind(regs_full_women, rep("Age+year+educ+region+marital", 20)))
...
binded %>%
  ...
  mutate(Race=factor(rep(c("Black", "Other"), 40), levels=c("Black", "Other")),
         V4=factor(V4, levels=c("any_disc_haras", "respect_bin", "trustman_bin", "manvsemp_bin", "suphelp_bin", "supcares_bin", "wkfreedm_bin", "wkdecide_bin", "safehlth_bin", "safefrst_bin")),
         V5=factor(V5, levels=c("Age+year", "Age+year+educ+region+marital")),
...
```{r, warning=FALSE, message=FALSE}
dat %>%
  filter(class=="Capitalists" | class=="Petit bourgeoisie") %>%
  group_by(class) %>%
  summarise(mean_emps=round(mean(numemps, na.rm=T),1), 
            median_emps=round(median(numemps, na.rm=T),1),
            lower_IQR=round(quantile(numemps, 0.25, na.rm=T),1),
            upper_IQR=round(quantile(numemps, 0.75, na.rm=T),1))
```
