---
title: "Classed, gendered, and racialized inequities in hazardous working conditions in the GSS"
...
Additional details available in the GSS Data Explorer (https://gssdataexplorer.norc.org/) and in the GSS Cumulative Codebook (https://gss.norc.org/documents/codebook/gss_codebook.pdf).
...
#remove ballot b in 2002, as class questions weren't asked of respondents in that ballot in that year
#remove 421 respondents who are marked 'IAP' in 2006 and 2014 for QWL variables, even after making appropriate sample restrictions - according to GSS helpdesk, they weren't administered the questions/didn't complete the survey ('breakoffs')
...
#########code new variables and make sure other variables are coded correctly and will be treated appropriately by MICE (e.g., no character vars)
######
#
  ...
  mutate(educ=factor(ifelse(degree == "bachelor" | degree == "graduate", "College +", 
                              ifelse(degree== "high school", "HS", 
                                     ifelse(degree == "junior college", "JuCo", 
                                            ifelse(degree == "lt high school", "<HS", NA)))), levels=c("<HS", "HS", "JuCo", "College +")), 
         ...
         disc_haras_bin_unimp=disc_haras_bin, #make duplicated version of each variable that we wont impute values for to use in outcome regressions
         ...
         satjob1_bin=ifelse(satjob1=="not at all satisfied" | satjob1=="not too satisfied", 1, 0),
...
#this code tells mice not to impute these variables: not imputing variables with complete data or the unimputed versions of the outcome variables; don't impute chngtme_bin either, since it wasn't asked in 2018 (correlation with other variables should mitigate bias) 
...
#when imputing race_h, we want gender and class as predictors (class_gender) but not class_poc_gender because of collinearity; don't need to do this for 'poc' because it's passively imputed
#when imputing class, we want gender and poc as predictors (poc_gender) but not class_poc_gender because of collinearity
...
![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-6-1.png)<!-- -->
...
```r
#regression function
mysvy <- function(imp_dat, outvars, adjvars, ...){
  model <- lapply(as.list(outvars), function(x){
    with(imp_dat, svyglm(as.formula(paste0(names(imp_dat$designs[[1]]$variables)[which(names(imp_dat$designs[[1]]$variables)==x)], adjvars)), family=poisson(), ...))
  })
  return(model)
}
...
#table function
tabled <- function(ests, captioned){
  kable(ests, digits=2, col.names=c("QWL variable", "Class", "PR", "Lower", "Upper"), caption=captioned, row.names=FALSE) %>%
...
## Classed, gendered, and racialized differences in QWL 
...
```r
#figure
plotted(binded_overall, limitsvec=c(0.088, 3.73), breaksvec=c(0.125, 0.25, 0.5, 1, 2, 4), 
             cols=brewer.pal(8, "Blues")[c(5,6,7,8)], shapes=c(15,16,17,18)) + theme(strip.text=element_text(colour = "black", vjust=1.07)) +
    plotted(binded_gender, limitsvec=c(0.088, 3.73), breaksvec=c(0.125, 0.25, 0.5, 1, 2, 4), 
                 cols=c(brewer.pal(8, "Blues")[c(5,6,7,8)], brewer.pal(8, "Purples")[c(5,6,7,8)]), shapes=c(15,16,17,18,0,1,2,5)) +
    plotted(binded_race, limitsvec=c(0.088, 3.73), breaksvec=c(0.125, 0.25, 0.5, 1, 2, 4), 
            cols=c(brewer.pal(8, "Blues")[c(5,6,7,8)], brewer.pal(8, "Purples")[c(5,6,7,8)]), shapes=c(15,16,17,18,0,1,2,5)) + 
...
![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-9-1.png)<!-- -->
...
![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-11-1.png)<!-- -->
...
```r
#figure
plotted(binded_overall, limitsvec=c(0.0259, 2.72), breaksvec=c(0.031, 0.0625, 0.125, 0.25, 0.5, 1, 2, 4), 
             cols=brewer.pal(8, "Blues")[c(5,6,7,8)], shapes=c(15,16,17,18)) + theme(strip.text=element_text(colour = "black", vjust=1.07)) +
    plotted(binded_gender, limitsvec=c(0.0259, 2.72), breaksvec=c(0.031, 0.0625, 0.125, 0.25, 0.5, 1, 2, 4), 
                 cols=c(brewer.pal(8, "Blues")[c(5,6,7,8)], brewer.pal(8, "Purples")[c(5,6,7,8)]), shapes=c(15,16,17,18,0,1,2,5)) +
    plotted(binded_race, limitsvec=c(0.0259, 2.72), breaksvec=c(0.031, 0.0625, 0.125, 0.25, 0.5, 1, 2, 4), 
            cols=c(brewer.pal(8, "Blues")[c(5,6,7,8)], brewer.pal(8, "Purples")[c(5,6,7,8)]), shapes=c(15,16,17,18,0,1,2,5)) + 
            xlab("Prevalence ratio") + theme(axis.title.x=element_text(), axis.text.x=element_text(), axis.ticks.x=element_line(), axis.title.y=element_blank()) +
  plot_layout(ncol=1, heights=c(1, 2, 2))
```

![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-13-1.png)<!-- -->
...
![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-15-1.png)<!-- -->
...
![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-17-1.png)<!-- -->
...
Note that the numemps variable wasn't asked in 2002, which is one reason we didn't use it for our primary analyses. Note also that numemps was only asked of people who identified as self-employed, which supports our use of the self-employed variable to identify owners.
...
### Confirming that binary QWL variables are coded correctly
...
```r
#class variable 
kable(table(dat$class, dat$wrkslf, dat$wksup, dat$occ10=="chief executives", useNA='always'), caption="Class by self employed by supervisor by chief executive", col.names=c("Class", "Self-employed", "Supervisor", "Chief executive", "Count")) %>%
  kable_styling("striped") %>%
  scroll_box(width = "100%", height = "250px")
```
...
kable(table(dat$class_gender, dat$sex, useNA='always'), caption="Class-gender by gender", col.names=c("Class-gender", "Gender", "NA")) %>%
  kable_styling("striped") %>%
  scroll_box(width = "100%", height = "250px")
```
...
```r
#class-POC variable
kable(table(dat$class_poc, dat$poc, useNA='always'), caption="Class-POC by POC") %>%
  kable_styling("striped") %>%
  scroll_box(width = "100%", height = "250px")
```
...
kable(table(dat$poc_gender, dat$race_h, dat$sex, useNA='always'), caption="Race-gender by race and gender", col.names=c("Race-gender", "Race", "Gender", "Count")) %>%
  kable_styling("striped") %>%
  scroll_box(width = "100%", height = "250px")
```
...
![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-23-1.png)<!-- -->
...
![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-24-1.png)<!-- -->![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-24-2.png)<!-- -->![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-24-3.png)<!-- -->![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-24-4.png)<!-- -->![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-24-5.png)<!-- -->![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-24-6.png)<!-- -->![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-24-7.png)<!-- -->![](analysis_11_9_21_MICE_files/figure-html/unnamed-chunk-24-8.png)<!-- -->
...
#functions
mysvy <- function(mi_dat, columns, adjvars, ...) {
  model <- lapply(as.list(columns), function(x) {
    svyglm(as.formula(paste0(names(mi_dat$variables)[which(names(mi_dat$variables)==x)], adjvars)), family=poisson(), design=mi_dat)
  })
  return(model)
}
...
tabled <- function(ests, captioned){
  kable(ests, digits=2, col.names=c("PR", "Lower", "Upper", "QWL variable", "Class"), caption=captioned, row.names=FALSE) %>%
    kable_styling("striped") %>%
    group_rows("Class", 1, 12) %>%
    group_rows("Class*gender", 13, 40) %>%
    group_rows("Class*POC", 41, 68) %>%
...
<tr>
  ...
  <tr>
   <td style="text-align:right;padding-left: 2em;" indentlevel="1"> 1.25 </td>
   <td style="text-align:right;"> 0.66 </td>
